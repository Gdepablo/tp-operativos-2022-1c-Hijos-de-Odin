...
void* hilo_suspensor(void* bloqueado_void){
    ...
    sem_wait(&esperando_respuesta_memoria); // asi otros no hacen sends a memoria hast que esto termina xdd
    ...
    while(1){
        ...
        sem_wait(&suspendiendo);
            if(bloqueado->esta_suspendido == 1){
            	// SI ESTA SUSPENDIDO: DISMINUYE EL GRADO ACTUAL DE MULTIPROGRAMACION, POST A PROCESOS PARA READY
